...
const CotizadorTesla30 = () => {
  ...
  const handleDescargar = async (formato) => {
    if (!cotizacion) {
      setError('No hay una cotización para descargar.');
      return;
    }
    ...
    try {
      // PASO 1: Si la cotización NO tiene ID, guardarla primero
      let cotizacionId = cotizacion.id;

      if (!cotizacionId) {
        console.log('📝 Guardando cotización en el backend primero...');
        
        // Preparar datos para el backend
        const datosParaBackend = {
          cliente: cotizacion.cliente || 'Cliente',
          proyecto: cotizacion.proyecto || 'Proyecto',
          descripcion: cotizacion.descripcionGeneral || '',
          items: cotizacion.items || [],
          subtotal: cotizacion.subtotal || 0,
          igv: cotizacion.igv || 0,
          total: cotizacion.total || 0,
          observaciones: cotizacion.observaciones || '',
          vigencia: cotizacion.validez || '30 días',
          estado: 'borrador'
        };

        // Guardar en el backend
        const response = await fetch('http://localhost:8000/api/cotizaciones/', {
          method: 'POST',
          headers: {
        ...
        const cotizacionGuardada = await response.json();
        cotizacionId = cotizacionGuardada.id;
        
        // Actualizar el estado con la cotización guardada
        setCotizacion({ ...cotizacion, id: cotizacionId, numero: cotizacionGuardada.numero });
        
        console.log(`✅ Cotización guardada con ID: ${cotizacionId}`);
      }

      // PASO 2: Generar el documento
      console.log(`📄 Generando ${formato.toUpperCase()} para cotización ${cotizacionId}`);
      setExito(`Generando ${formato.toUpperCase()}...`);
      
      // Llamar a la función de descarga
      const result = await descargarInforme(cotizacionId, formato);
