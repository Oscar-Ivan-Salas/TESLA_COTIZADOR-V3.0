...
**¿Qué sistema necesitas?**`,
    ...
    const enviarMensaje = async (mensaje) => {
        ...
        setIsTyping(true);
        try {
            const res = await fetch('http://localhost:8000/api/chat/pili-domotica', {
                method: 'POST',
    ...
    return (
        <div className="flex flex-col h-full bg-gradient-to-br from-purple-900 via-purple-800 to-purple-950 rounded-2xl shadow-2xl overflow-hidden border-2 border-purple-600">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-purple-950 to-black">
                {conversacion.map((msg, i) => (
                    <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className="max-w-[80%]">
                            <div className={`rounded-2xl p-4 shadow-lg ${msg.sender === 'user' ? 'bg-gradient-to-br from-pink-500 to-pink-600 text-purple-900' : 'bg-gradient-to-br from-purple-800 to-purple-900 text-white border-2 border-purple-600'}`}>
                                <div dangerouslySetInnerHTML={{ __html: (msg.text || '').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br />') }} />
                            </div>
                            {msg.buttons && (
                                <div className="mt-3 flex flex-wrap gap-2">
                                    {msg.buttons.map((btn, j) => (
                                        <button key={j} onClick={() => { addMessage('user', btn.value); setTimeout(() => enviarMensaje(btn.value), 100); }} disabled={isTyping} className="px-4 py-2 bg-gradient-to-r from-pink-500 to-pink-600 hover:from-pink-400 hover:to-pink-500 text-purple-900 rounded-lg font-semibold shadow-md transition-all transform hover:scale-105 disabled:opacity-50">
                                            {btn.text}
                                        </button>
                                    ))}
                                </div>
                            )}
                            <div className={`text-xs mt-1 ${msg.sender === 'user' ? 'text-right text-purple-300' : 'text-left text-purple-400'}`}>{msg.timestamp}</div>
                        </div>
                    </div>
                ))}
                {isTyping && (
                    <div className="flex justify-start">
                        <div className="bg-purple-800 rounded-2xl p-4 border-2 border-purple-600">
                            <div className="flex gap-2">
                                {[0, 150, 300].map((delay, i) => (
                                    <div key={i} className="w-2 h-2 bg-pink-400 rounded-full animate-bounce" style={{ animationDelay: `${delay}ms` }}></div>
                                ))}
                            </div>
                        </div>
                    </div>
            ...
            <div className="bg-purple-900 p-4 border-t-2 border-pink-500">
                <div className="flex gap-2">
                    <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), inputValue.trim() && (addMessage('user', inputValue.trim()), enviarMensaje(inputValue.trim()), setInputValue('')))} placeholder="Escribe tu mensaje..." disabled={isTyping} className="flex-1 px-4 py-3 bg-purple-800 text-white border-2 border-purple-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 placeholder-purple-400 disabled:opacity-50" />
                    <button onClick={() => { if (inputValue.trim()) { addMessage('user', inputValue.trim()); enviarMensaje(inputValue.trim()); setInputValue(''); } }} disabled={!inputValue.trim() || isTyping} className="px-6 py-3 bg-gradient-to-r from-pink-500 to-pink-600 hover:from-pink-400 hover:to-pink-500 text-purple-900 rounded-lg font-semibold shadow-lg transition-all transform hover:scale-105 disabled:opacity-50 flex items-center gap-2">
                        <Send className="w-5 h-5" />
                        Enviar
                    </button>
                </div>
                {hasQuote && onFinish && (
                    <button onClick={onFinish} className="w-full mt-3 px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-500 hover:to-green-600 text-white rounded-lg font-bold shadow-lg transition-all transform hover:scale-105">
                        ✅ Finalizar y Ver Cotización
                    </button>
                )}
            </div>

            <div className="bg-purple-950 p-3 border-t border-purple-800">
...
export default PiliDomoticaChat;
